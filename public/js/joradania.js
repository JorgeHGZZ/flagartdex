...
function showStats() {
    ...
    const Uniforme = crearBoton3d('Uniforme', 0xffffff, 0, 0, 'Uniforme_jordania.jpg');
    Uniforme.position.set(0.08, -0.03, -0.2);
    Uniforme.scale.set(0.9, 0.6, 0.4);
...
function showVideo() {
    ...
    video.src = '/assets/videos/jordania.mp4';
    video.loop = true;
    video.muted = false;
...
function showModel() {
    ...
    loader.load('/assets/models/jordan.glb', (gltf) => {
        currentModel = gltf.scene; // ✅ guardar referencia aquí
        currentModel.position.set(0, -0.5, -2);
        currentModel.rotation.y = -90;
        currentModel.scale.set(2.5, 2.5, 2.5);
